...
### Best Practices: GPU/CPU Discipline in AI Pipelines
- **Don't use an LLM when you don't need to.** This is the single most actionable takeaway from the project. Every pipeline step should have an explicit type: GPU (LLM call) or CPU (deterministic logic). If a step can be CPU, it must be CPU. This is evidence-based and measurable.
- **The execution plan as artifact is the unique contribution.** Multiple LLM feedback sessions independently identified this as the distinctive element — not the DSL syntax, not the verbs, but the fact that you get a inspectable, diffable plan *before* runtime that shows exactly where intelligence is used vs where logic suffices.
- **This is profiler-adjacent, not a profiler.** Like SQL Profiler showing query costs per step, an "LLM pipeline profiler" would show: which steps hit the model, what they cost, and which could be pushed to CPU. The DSL's execution plan is a static version of this. A dynamic version (instrument actual agent pipelines, flag LLM-heavy steps) is a natural tool idea — but will likely emerge from coding agent platforms or refactoring tools rather than as a standalone product.
- **HITL (human-in-the-loop) as a verb type matters for compliance.** Beyond GPU and CPU, a third category: steps that require human review before proceeding. REVIEW, APPROVE, ESCALATE — these are real operations in regulated pipelines. Declaring them explicitly in the spec (not hiding them in code) is what makes the pipeline auditable.
- **Content opportunity: blog post or article.** The GPU/CPU/HITL breakdown, execution plan as audit artifact, and "don't over-use LLMs" message are all strong content pieces. Practical, opinionated, and backed by a working implementation. Worth writing up regardless of product direction.
